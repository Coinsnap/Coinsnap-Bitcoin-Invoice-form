...
class CoinsnapProvider implements PaymentProviderInterface {
	...
	$api_key = (string) $settings['coinsnap_api_key'];
	$store   = (string) $settings['coinsnap_store_id'];
        $webhook_url = rtrim( (string) get_home_url(), '/' ) . '/wp-json/' . CoinsnapBIF_Constants::REST_NAMESPACE . CoinsnapBIF_Constants::REST_ROUTE_WEBHOOK_COINSNAP;
	
        if ( ! $api_base || ! $api_key || ! $store ) {
            return false;
        }

        $webhookOption = get_option( Settings::WEBHOOK_KEY );
                ...
                //  If webhook is registered this site    
                    if(strpos( $webhook['url'], $webhook_url ) !== false){
                        ...
                        //  If not - we delete this webhook from server
                            ...
                            $url     = $api_base . sprintf( CoinsnapBIF_Constants::COINSNAP_WEBHOOKS_ENDPOINT_V1, rawurlencode( $store ) ) . '/' . $webhook['id'];
                            $args    = array(
                                'method'  => 'DELETE',
                                'headers' => array(
                                    'X-Api-Key'     => $api_key,
	...
	$store   = (string) $settings['coinsnap_store_id'];
        ...
        $webhook_url = $base_url . '/wp-json/' . CoinsnapBIF_Constants::REST_NAMESPACE . CoinsnapBIF_Constants::REST_ROUTE_WEBHOOK_COINSNAP;
