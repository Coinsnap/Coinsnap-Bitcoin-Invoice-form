...
class BTCPayProvider implements PaymentProviderInterface {
	...
	$api_key = (string) $s['btcpay_api_key'];
	$store   = (string) $s['btcpay_store_id'];
        $ngrok_url   = ! empty( $s['ngrok_url'] ) ? rtrim( $s['ngrok_url'], '/' ) : '';
        $base_url    = $ngrok_url ? $ngrok_url : rtrim( (string) get_home_url(), '/' );
        $webhook_url = $base_url . '/wp-json/' . CoinsnapBIF_Constants::REST_NAMESPACE . CoinsnapBIF_Constants::REST_ROUTE_WEBHOOK_BTCPAY;
