...
class CoinsnapBIF_Services_Payment_Service {
	...
	public static function create_invoice( int $form_id, array $data ): array {
		...
		try {
			...
			// Apply global Bitcoin discount if enabled in plugin settings.
			$global_settings = Settings::get_settings();
			if ( ! empty( $global_settings['bitcoin_discount_enabled'] ) ) {
				$global_type  = $global_settings['bitcoin_discount_type'] ?? 'percentage';
				$global_value = (float) ( $global_settings['bitcoin_discount_value'] ?? 0.0 );
				if ( $global_value > 0 ) {
					if ( 'percentage' === $global_type ) {
						$amount = $amount - ( $amount * ( $global_value / 100 ) );
					} else { // fixed
						$amount = $amount - $global_value;
					}
					if ( $amount < 0 ) {
						$amount = 0;
					}
