...
/**
 * Plugin Name: Bitcoin Invoice Form (Coinsnap & BTCPay Integration)
...
define( 'BIF_VERSION', '0.1.0' );
define( 'BIF_PLUGIN_FILE', __FILE__ );
define( 'BIF_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'BIF_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
...
spl_autoload_register(
	function ( $autoload_class ) {
		...
		if ( isset( $class_mapping[ $relative ] ) ) {
			$file = BIF_PLUGIN_DIR . 'src/' . $class_mapping[ $relative ] . '.php';
		} else {
			// Default PSR-4 mapping.
			$file = BIF_PLUGIN_DIR . 'src/' . str_replace( '\\', '/', $relative ) . '.php';
		}
...
register_activation_hook(
	...
	function () {
		if ( ! class_exists( 'BitcoinInvoiceForm\\Database\\Installer' ) ) {
			require_once BIF_PLUGIN_DIR . 'src/Database/class-installer.php';
		}
...
add_action(
	...
	function () {
		if ( ! class_exists( 'BitcoinInvoiceForm\\BIF_Plugin' ) ) {
			require_once BIF_PLUGIN_DIR . 'src/class-bif-plugin.php';
		}
